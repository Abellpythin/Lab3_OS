...
int** read_board_from_file(char* filename)
    // Creating a function that will rea the board
    {
    // Declaring a pointer to int as board
    int** board = NULL;
    
    // replace this comment with your code
    FILE *fp = fopen(filename, "r");
    // Allocaing mem for row_size pointers and matching board type
    board = (int**)malloc(ROW_SIZE * sizeof(int*));
    for(int i= 0;i<ROW_SIZE;i++) {
        // Allocating mem for each coloum in row
        board[i] =(int*) malloc(COL_SIZE*sizeof(int));
    }   // Checking if the file is unable to open
    if (fp == NULL){
...
void* validate(void* p) {
    ...
    // Nested loop to check the values
